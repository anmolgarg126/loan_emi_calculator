...
The first release will not reproduce a named lender product, fetch live rates, model construction-stage disbursements or pre-EMI, calculate tax benefits, underwrite affordability, synchronize data to the cloud, or provide financial advice. It will not embed charts in XLSX files.
...
- `src/lib`: URL-fragment persistence, local storage, printing, CSV, and XLSX export.
...
The OD state keeps these values separate:

- **Principal outstanding:** contractual loan principal not permanently repaid.
- **Drawing power:** maximum permitted debit balance after scheduled reductions.
- **Parked surplus:** user funds deposited into the OD account and still withdrawable.
- **Available withdrawal:** parked surplus that can currently be withdrawn without exceeding drawing power.
- **Net interest-bearing balance:** `max(principal outstanding - parked surplus, 0)`.

OD deposits never become prepayments. A prepayment permanently reduces principal and drawing power and cannot later be withdrawn.
...
- Currency: INR with integer paise stored internally.
...
3. Apply the scheduled transfer, first to posted interest, and reduce principal outstanding and drawing power by the contractual principal component.
...
8. Calculate interest for the current day on the closing net interest-bearing balance.
...
The scheduled OD transfer equals the standard EMI calculated for full utilization. Its contractual principal component reduces principal outstanding and drawing power. Actual daily OD interest is charged separately; when the scheduled transfer exceeds actual interest plus the contractual principal reduction, the difference increases parked surplus and remains withdrawable.
...
The effective OD rate is the active standard rate plus the OD premium. The premium applies to the full net interest-bearing OD balance, not only to withdrawals beyond the original principal.

## User interface

The app is one responsive page. Essential loan inputs appear first. Homeowner costs, prepayments, rate changes, OD, and arbitrary OD transactions are optional sections. Rate changes and prepayments select an EMI cycle; arbitrary OD transactions use native calendar dates. Enabling OD reveals the premium, effective rate, fees, opening parked surplus in rupees or percentage of original principal, and monthly contribution. Enabling arbitrary transactions reveals rows with date, deposit/withdrawal type, amount, and remove action.

The calculated loan amount is read-only. Every percentage states its basis. Terminology uses “parked surplus,” “available withdrawal,” and “net interest-bearing balance,” avoiding the ambiguous phrase “OD balance.” Native date inputs, visible focus, keyboard operation, inline errors, Reset, and Example actions are required.

## Results

The primary comparison shows standard EMI, effective OD rate, standard interest, OD interest, OD fees, fee-adjusted savings, break-even parked surplus, and payoff dates. Supporting balances show principal, drawing power, parked surplus, available withdrawal, and net interest-bearing balance.
...
- Homeowner expenses during the modelled tenure.
...
Shared scenarios use a versioned URL fragment, not query parameters. The fragment is validated before use and is not automatically transmitted to the host. Sharing is always user-initiated.

Named scenarios use a versioned local-storage schema. Users can load, delete, or clear saved scenarios. Storage failures do not prevent calculation. The privacy text states that saved scenarios may be visible to anyone using the same browser profile.
...
Validation rejects missing required values, negative money values where not meaningful, invalid tenure or rate changes, rate changes or prepayments outside an EMI cycle, OD transactions before loan start or after payoff, withdrawals above available surplus, unsupported share versions, unsafe numeric magnitudes, and EMIs that cannot cover interest. V1 accepts home value and principal above zero up to ₹100 crore, other money values from zero to ₹100 crore, tenure from 1 to 480 months, standard annual rates from 0% to 50%, OD premiums from 0% to 20%, and ordinary percentage inputs from 0% to 100%. A keep-EMI rate reset that would extend calculation beyond 600 months is rejected with a warning to increase EMI. These are calculator safety limits, not lending eligibility rules.

Errors appear beside the responsible input. Invalid calculations do not replace the last valid results. Malformed shared fragments are ignored with a warning. Local storage, print, CSV, and XLSX failures produce recoverable messages and never corrupt the active scenario.

## Testing

Vitest covers golden standard-loan cases, OD-disabled equality, Actual/365 accrual, same-day event ordering, both rate-reset modes, infeasible EMI detection, opening and monthly surplus, arbitrary deposits and withdrawals, withdrawal caps, prepayment separation, leap dates, early payoff, malformed fragments, and reconciliation invariants.
...
One Playwright smoke test covers the primary desktop and mobile flow, OD and transaction toggles, rate-reset selection, sharing, local save/delete, print preparation, CSV, and XLSX download initiation.

## Deployment

Vite builds static files into `dist`. GitHub Actions runs lint, type-check, tests, and build before uploading `dist` to GitHub Pages from `main`. The project-site base path is `/loan_emi_calculator/`. A verified custom domain changes the base to `/` and uses GitHub Pages HTTPS. Rollback redeploys a previously verified commit.
