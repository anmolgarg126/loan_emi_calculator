  ...
  return <div className="guided-form">
    <GuidedSection step={1} title="Home loan essentials" description="Property value, contribution, loan rate, and term" open>
      <div className="field-grid">
        ...
        <div className="field-with-mode"><NumberField id="down-payment" label="Down payment" value={scenario.downPayment} onChange={(value) => set('downPayment', value)} prefix={scenario.downPaymentMode === 'amount' ? '₹' : undefined} suffix={scenario.downPaymentMode === 'percent' ? '%' : undefined} error={issueFor('downPayment')} /><ModeToggle id="down-payment-mode" value={scenario.downPaymentMode} onChange={(value) => set('downPaymentMode', value)} percentLabel="% of home" error={issueFor('downPaymentMode')} /></div>
        ...
        <label className="field" htmlFor="loan-amount"><span className="field-label">Calculated loan amount</span><span className="input-shell read-only"><span className="input-affix">₹</span><input id="loan-amount" aria-label="Calculated loan amount" value={Math.round(result.loanAmount)} readOnly /></span><small>Home value + financed insurance − down payment</small></label>
    ...
    <GuidedSection step={2} title="Ownership and lender costs" description="Fees and ongoing costs outside principal" optional configured={scenario.processingFee + scenario.oneTimeExpenses + scenario.propertyTaxAnnual + scenario.homeInsuranceAnnual + scenario.maintenanceMonthly > 0}>
      <div className="field-grid">
        ...
        ] as const).map(([field, mode, label, percentLabel]) => <div className="field-with-mode" key={field}><NumberField id={field} label={label} value={scenario[field]} onChange={(value) => set(field, value)} prefix={scenario[mode] === 'amount' ? '₹' : undefined} suffix={scenario[mode] === 'percent' ? '%' : undefined} error={issueFor(field)} /><ModeToggle id={mode} value={scenario[mode]} onChange={(value) => set(mode, value)} percentLabel={percentLabel} error={issueFor(mode)} /></div>)}
